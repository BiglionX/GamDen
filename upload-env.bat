...
set OUT=d:\GamDen\upload-env-result.txt

echo === Upload .env to server === > %OUT%
scp %SSHOPTS% d:\GamDen\.env %HOST%:/opt/GamDen/.env >> %OUT% 2>&1
echo. >> %OUT%
echo === Verify .env on server === >> %OUT%
ssh %SSHOPTS% %HOST% "ls -la /opt/GamDen/.env && echo --- && head -25 /opt/GamDen/.env && echo --- && echo check duplicates: && grep -nE '^(DB_HOST|DB_PORT|REDIS_HOST)=' /opt/GamDen/.env" >> %OUT% 2>&1
echo. >> %OUT%
...
echo. >> %OUT%
echo === Wait 35s and check logs === >> %OUT%
ssh %SSHOPTS% %HOST% "sleep 35 && docker logs --tail 100 gamden-backend 2>&1" >> %OUT% 2>&1
echo. >> %OUT%
