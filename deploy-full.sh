...
# 端口：marketing:3002  admin:3001  backend:3003
# ============================================
...
# --------------------------------------------
# Step 1: 安装 Nginx + 清理冲突的容器/进程
# --------------------------------------------
echo "[1/6] 安装 Nginx 并清理冲突..."

# 停止并禁用可能占用 80 端口的旧 gamden-nginx 容器（之前部署残留）
ssh $SERVER "docker stop gamden-nginx 2>/dev/null; docker update --restart=no gamden-nginx 2>/dev/null; docker rm -f gamden-nginx 2>/dev/null; true"

# 杀掉所有宿主机 nginx 孤儿进程，释放 80 端口
ssh $SERVER "pkill -9 nginx 2>/dev/null; sleep 2; true"
