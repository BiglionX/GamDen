...
set OUT=d:\GamDen\decode-env-result.txt

echo === Decode base64 and write to /opt/GamDen/.env === > %OUT%
ssh %SSHOPTS% %HOST% "rm -f /opt/GamDen/.env && touch /opt/GamDen/.env && base64 -d /tmp/.env.b64 > /opt/GamDen/.env 2>&1 && wc -l /opt/GamDen/.env && wc -c /opt/GamDen/.env && rm /tmp/.env.b64" >> %OUT% 2>&1
echo. >> %OUT%
echo === Verify duplicate check === >> %OUT%
...
echo. >> %OUT%
echo === First 30 lines === >> %OUT%
ssh %SSHOPTS% %HOST% "head -30 /opt/GamDen/.env" >> %OUT% 2>&1
echo. >> %OUT%
...
ssh %SSHOPTS% %HOST% "sleep 30 && docker logs --tail 60 gamden-backend 2>&1" >> %OUT% 2>&1
echo. >> %OUT%
